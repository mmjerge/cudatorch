 ...
 */
 ...
 // Tensor cores implementation using WMMA (Warp Matrix Multiply Accumulate)
 __global__ void matrixMulTensorCores(half *a, half *b, float *c, int n) {
     // Using WMMA from the nvcuda::wmma namespace
 ...
 bool verifyResults(float *a, float *b, float *c, int n, bool isTensorCore = false) {
     ...
     const float epsilon = isTensorCore ? 0.1f : 1e-2;
     
     for (int i = 0; i < n; i++) {
         for (int j = 0; j < n; j++) {
             if (fabs(verification[i * n + j] - c[i * n + j]) > epsilon) {
                 printf("Verification failed at [%d, %d]: Expected %f, got %f (diff: %f)\n", 
                        i, j, verification[i * n + j], c[i * n + j], 
                        fabs(verification[i * n + j] - c[i * n + j]));
                 free(verification);
                 return false;
             }
         }
 ...
 int main() {
     ...
     if (deviceProp.major >= 7) {  
         ...
         // Verify result (with higher tolerance due to half precision)
         printf("Verification %s\n", verifyResults(h_a, h_b, h_c, N, true) ? "PASSED" : "FAILED");
     } else {
